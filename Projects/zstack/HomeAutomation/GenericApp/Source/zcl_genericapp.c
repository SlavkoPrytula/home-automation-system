...
endPointDesc_t zclGenericApp_epDesc;
...
// NEW
byte zclGenericApp_TransID;  // This is the unique message ID (counter)
...
// NEW
void zclGenericApp_ProcessZDOMsgs( zdoIncomingMsg_t *inMsg );
void zclGenericApp_MessageMSGCB( afIncomingMSGPacket_t *pckt );
void zclGenericApp_SendTheMessage( unsigned char dest_endID, unsigned char cmd, unsigned int temp_set);
void zclGenericApp_SerialMSGCB(void);
...
void zclGenericApp_Init( byte task_id )
{
  ...
  halUARTCfg_t uartConfig;				// NEW
  zclGenericApp_TaskID = task_id;
  zclGenericApp_NwkState = DEV_INIT;	// NEW
  zclGenericApp_TransID = 0;				// NEW
  ...
  zclGenericApp_epDesc.endPoint = GENERICAPP_ENDPOINT;
  zclGenericApp_epDesc.task_id = &zclGenericApp_TaskID;
  zclGenericApp_epDesc.simpleDesc
            = (SimpleDescriptionFormat_t *)&GenericApp_SimpleDesc;
  zclGenericApp_epDesc.latencyReq = noLatencyReqs;

  // Register the endpoint description with the AF
  afRegister( &zclGenericApp_epDesc );
  ...
  if ( events & SYS_EVENT_MSG )
  {
	...
	case AF_DATA_CONFIRM_CMD:
        ...
        case AF_INCOMING_MSG_CMD:
          zclGenericApp_MessageMSGCB( MSGpkt );
          break;
	  // ENDNEW
      ...
      // Next
      MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( zclGenericApp_TaskID );		// NEW
    }
...
/*********************************************************************
 * @fn      zclGenericApp_ProcessZDOMsgs()
    ...
    case Match_Desc_rsp:
      {
        ...
        if ( pRsp )
        {
          if ( pRsp->status == ZSuccess && pRsp->cnt )
          {
            ...
            zclGenericApp_DstAddr.addr.shortAddr = pRsp->nwkAddr;
 ...
 * @return  none
 */
void zclGenericApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
      ...
      {// "the" message
        ...
        for (i = 0; i < device_count+1; i++) {  // to see whether the device ever connected
          if (osal_memcmp(EndDeviceInfos[i].extAddr, temp_extAddr, 8)) { //if existed, break;
            existed = 1;
            HalUARTWrite(0, "Device Found Form Previous Log -> Existed!\r\n", 36);
            break;
          }
        }
...
/*********************************************************************
 * @fn     zclGenericApp_SendTheMessage
 * @brief   Send "the" message.
 * @param   none
 * @return  none
 */
void zclGenericApp_SendTheMessage( unsigned char dest_endID, unsigned char cmd, unsigned int data)
{
  ...
  zclGenericApp_DstAddr.addrMode = (afAddrMode_t)Addr64Bit;
  zclGenericApp_DstAddr.endPoint = EndDeviceInfos[dest_endID].endPoint;
  osal_memcpy(zclGenericApp_DstAddr.addr.extAddr, EndDeviceInfos[dest_endID].extAddr, 8);
  
  if ( AF_DataRequest( &zclGenericApp_DstAddr, &zclGenericApp_epDesc,
                       GENERICAPP_CLUSTERID,
                       ...
                       AF_DISCV_ROUTE, AF_DEFAULT_RADIUS ) == afStatus_SUCCESS )                     
  {
    ...
    HalUARTWrite(0, "Successfully Sent\r\n", 21);
  }
  else
  {
 ...
 // printf("UART received!");
  ...
  if ( (buf[1] & GENERICAPP_ENDPOINT) && (buf[0] == 0xCC) && (buf[5] == 0x33)) {  //make sure cmd send to this device
    ...
    if (buf[2]&0x80) {  //cmd for coordinator
      ...
      zclGenericApp_SendTheMessage(dest_endID, buf[2], data);
    }
  }
}
