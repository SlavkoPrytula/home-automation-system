...
static zclGeneral_AppCallbacks_t zclSampleLight_CmdCallbacks =
{
  ...
  zclSampleLight_IdentifyCB,                                  	// Identify Trigger Effect command
...
//          UI_DeviceStateUpdated((devStates_t)(MSGpkt->hdr.status));
	  ...
	  // now on the network
          if ( (zclSampleLight_NwkState == DEV_ZB_COORD) || (zclSampleLight_NwkState == DEV_ROUTER) || (zclSampleLight_NwkState == DEV_END_DEVICE) )
          {
           	// now on network		  
...
//	  HalLedSet( HAL_LED_ALL, HAL_LED_MODE_TOGGLE);
//	  osal_start_timerEx( zclSampleLight_TaskID, DIYRuZRT_EVT_1, 500 );
 ...
 *                 HAL_KEY_SW_2
 *                 HAL_KEY_SW_1
 *		   HAL_KEY_SW_6 -- Button S1 if available
*		   HAL_KEY_SW_7 -- Button S2 if available
 *
 * @return  none
 */
static void zclSampleLight_HandleKeys( byte shift, byte keys )
